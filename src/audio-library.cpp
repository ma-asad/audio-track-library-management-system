...
void AudioLibrary::listAudio()
{
    ...
    tracks.getItems(items, counter); // Retrieve the audio tracks from the hash table
...
void AudioLibrary::quickSort(std::pair<std::string, AudioTrack> *items, int low, int high)
{
    if (low < high)
    {
        int pivotIndex = partition(items, low, high); // Partition the array and get the pivot index
        quickSort(items, low, pivotIndex - 1);        // Recursively sort the left subarray
        quickSort(items, pivotIndex + 1, high);       // Recursively sort the right subarray
    }
}

// Partition function for Quicksort
int AudioLibrary::partition(std::pair<std::string, AudioTrack> *items, int low, int high)
{
    std::string pivot = items[high].first; // Use the key (audio name) of the last element as the pivot
    ...
    for (int j = low; j < high; ++j)
    {
        ...
        if (toLowercase(items[j].first) <= toLowercase(pivot))
        {
            ...
            std::swap(items[i], items[j]); // Swap the current element with the smaller element
        }
    }

    std::swap(items[i + 1], items[high]); // Swap the pivot with the element at index i+1
...
std::string AudioLibrary::toLowercase(const std::string &str)
{
    ...
    // Transform each character in the string to lowercase using std::transform and std::tolower
