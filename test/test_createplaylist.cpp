...
TEST_CASE("Creating and Managing Playlists") {
    ...
    SECTION("Create a new playlist") {
        std::istringstream input("My Playlist\nCampaign eye interview\nCall\n0\n0\n");
        ...
        REQUIRE(playlistTracks.second[0].second.getAudioName() == "Campaign eye interview");
    ...
    SECTION("Add tracks to an existing playlist") {
        std::istringstream input("Existing Playlist\nCampaign eye interview\nCall\n0\n0\n");
        std::cin.rdbuf(input.rdbuf());
        library.createPlaylist();
